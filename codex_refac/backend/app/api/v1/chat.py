...
async def process_message(message: ChatMessage, request: Request):
    ...
    try:
        # Fast-path for CI/testing to avoid external dependencies
        if os.getenv("FAST_TEST", "0").lower() in ("1", "true", "yes"):
            session_id = message.session_id or str(uuid.uuid4())
            return ChatResponse(
                session_id=session_id,
                response="Test response",
                products=[],
                suggestions=["Tell me more", "Show products"],
            )
        ...
        for p in ai_products:
            ...
            # Ensure a valid UUID for id
            try:
                pid = p.get('id')
                product_id = uuid.UUID(str(pid)) if pid else uuid.uuid4()
            except Exception:
