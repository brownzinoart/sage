...
# Use the consolidated CleanSageService across API
...
async def sage_health():
    ...
    health_info = {
        "status": "healthy",
        "service": "Sage AI (Gemini + optional research)",
...
async def specialized_research(query: ResearchQuery):
    ...
    try:
        ...
        from mcp_educational.mcp_types import ResearchQuery as MCPResearchQuery
...
# New: evidence-based dosage guidelines
class DosageQuery(BaseModel):
    compound: str
    condition: str
    experience_level: Optional[str] = "curious"  # maps to ranges in guidelines
    user_profile: Optional[Dict[str, Any]] = None

@router.post("/dosage")
async def evidence_dosage(query: DosageQuery):
    try:
        if not ENHANCED_AVAILABLE:
            raise HTTPException(status_code=503, detail="Enhanced research not available")

        user_profile = query.user_profile or {"experience_level": query.experience_level}
        result = await sage_service.educational_mcp._get_dosage_guidelines(
        ...
        print(f"Error in dosage guidelines: {e}")
        raise HTTPException(status_code=500, detail="Error getting dosage guidelines")

# New: drug interaction checker
class InteractionsQuery(BaseModel):
    compounds: List[str]
    medications: Optional[List[str]] = None

@router.post("/interactions")
async def check_interactions(query: InteractionsQuery):
    try:
        if not ENHANCED_AVAILABLE:
            raise HTTPException(status_code=503, detail="Enhanced research not available")

        result = await sage_service.educational_mcp._check_drug_interactions(
            compounds=query.compounds,
            medications=query.medications or []
        )
        recommendation = sage_service.educational_mcp._generate_interaction_recommendation(result.get('interactions', {}))
        return {"interactions": result, "recommendation": recommendation}
    except HTTPException:
        raise
    except Exception as e:
        print(f"Error in interaction check: {e}")
        raise HTTPException(status_code=500, detail="Error checking interactions")

# New: legal status and mechanism explainers
class LegalQuery(BaseModel):
    location: str = "federal"
    product_type: Optional[str] = None
    compounds: Optional[List[str]] = None

@router.post("/legal")
async def legal_status(query: LegalQuery):
    try:
        if not ENHANCED_AVAILABLE:
            raise HTTPException(status_code=503, detail="Enhanced research not available")
        result = await sage_service.educational_mcp._get_legal_status(
            location=query.location,
            product_type=query.product_type,
            compounds=query.compounds or []
        )
        return result
    except HTTPException:
        raise
    except Exception as e:
        print(f"Error in legal status: {e}")
        raise HTTPException(status_code=500, detail="Error getting legal status")

class MechanismQuery(BaseModel):
    compound: str
    target_system: Optional[str] = "endocannabinoid"
    detail_level: Optional[str] = "intermediate"

@router.post("/mechanism")
async def explain_mechanism(query: MechanismQuery):
    try:
        if not ENHANCED_AVAILABLE:
            raise HTTPException(status_code=503, detail="Enhanced research not available")
        result = await sage_service.educational_mcp._explain_mechanism(
            compound=query.compound,
        ...
        print(f"Error in mechanism explain: {e}")
        raise HTTPException(status_code=500, detail="Error explaining mechanism")
